...
export function Navbar () {
  ...
  const handleNavClick = (id: string) => {
    console.log('Navigating to section:', id) // Debugging log
    setMobileOpen(false)
    const target = document.getElementById(id)
    if (!target) return
    if (lenis) {
      lenis.scrollTo(target, { duration: 1.2, offset: -80 }) // matches scroll-mt-20 above
    } else {
        ...
        <nav className='hidden md:flex items-center gap-8'>
          {navLinks.map(link => (
            <button
              ...
              className={`font-handwritten cursor-pointer text-sm relative pb-1 transition-colors ${
                active === link.id
